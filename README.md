...
- 下载模式：镜像 / 仅新增 / 仅删除（作用于「歌单→本地」），可全局或按歌单配置，默认镜像；可选的“把手动放入的歌补进网易歌单”（仅新增、绝不反向删歌，默认关闭）
- 每次同步记录每条新增/删除变更，可逐曲恢复；自动保存歌单快照，可回滚到任一历史状态
- 歌单列表按「我创建的 / 我收藏的」分组；收藏歌单不可写入网易，自动单向同步
- 账号页展示网易等级、VIP、关注/粉丝、歌单/喜欢数与本地累计同步统计
- 独立 NCM 转换工具：选择文件或目录批量转换，可保留或删除源文件
...
4. 在“歌单同步”中开启需要同步的歌单，点击“立即同步全部”或等待自动任务；也可以在设置中为每个歌单（或全局）配置下载模式（镜像/仅新增/仅删除）。镜像模式下，本地多余、歌单已移除的文件会自动进入隔离区并在删除记录中可恢复；还可通过顶部“备份到本地目录”下载喜欢/已购歌曲。
5. 被歌单移除/本地多余的文件会进入 `<音乐根目录>/.quarantine/`，可在“隔离区”或“同步任务 → 可恢复的删除记录”中恢复或删除。
